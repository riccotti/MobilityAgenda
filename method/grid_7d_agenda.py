...
def build_grid_7d(train, history_order_dict, grid_length, time_lenght, min_lon, min_lat, tzoffset):
    ...
    for weekday in weekday_train:
        ...
        weekday_model[weekday] = grid_model

    return weekday_model


def generate_grid_7d_agenda(weekday, grid_7d_model, grid_length, time_lenght, min_lon, min_lat, tzoffset,
                            random_choice=False, time_start=0, time_offset=300, time_stop=86400, starting_cell=None):

    grid_model = grid_7d_model[weekday]
    if grid_model is None:
        if len(grid_7d_model) > 0:
            closest_weekday = np.min([abs(x - weekday) for x in grid_7d_model]) + weekday
            if closest_weekday in grid_7d_model:
                grid_model = grid_7d_model[closest_weekday]
            else:
                grid_model = grid_7d_model[grid_7d_model.keys()[0]]
...
def main():
    ...
    grid_7d_model = build_grid_7d(train, history_order_dict, grid_length, time_lenght, min_lon, min_lat, tzoffset)

    grid_7d_agenda, ending_cell = generate_grid_7d_agenda(weekday, grid_7d_model, grid_length, time_lenght,
                                                          min_lon, min_lat, tzoffset, random_choice=False,
                                                          time_start=0, time_offset=300, time_stop=86400)

    print '------'

    for event_time in sorted(grid_7d_agenda):
        event = grid_7d_agenda[event_time]
